{% macro fabricserverless__create_schema(relation) -%}
...
{% macro fabricserverless__create_schema_with_authorization(relation, schema_authorization) -%}
  {% call statement('create_schema') -%}
    {{ use_database_hint(relation.database) }}
    IF NOT EXISTS (SELECT * FROM sys.schemas WHERE name = '{{ relation.schema }}')
    BEGIN
...
{% macro fabricserverless__drop_schema(relation) -%}
